...
def gdrive(ctx):
    ...
    with open(os.devnull, 'w') as devnull:
        subprocess.Popen(['bash', './extension/startssh.sh', 'gdrive'], stdout=devnull, stderr=devnull)
    with open(os.devnull, 'w') as devnull:
        subprocess.Popen(['php', '-S', 'localhost:8080', '-t', 'template/gdrive/'] , stdout=devnull, stderr=devnull)
    BOT.send_message(ctx.chat.id, f"Your link is: https://gdrive.serveo.net")
...
<b>Tanggal :</b> {date}
<b>Dari link :</b> https://gdrive.serveo.net
